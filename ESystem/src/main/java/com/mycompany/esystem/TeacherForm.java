...
public class TeacherForm extends javax.swing.JFrame {
        ...
        try {
            ...
            try {
                ...
                if (!userExists) {
                    ...
                    System.out.println("Created new database user: " + username);
                }
                
                // Grant SELECT, INSERT, UPDATE privileges on the current database
                ...
                System.out.println("Granted privileges to " + username + " on database " + escapedDB);
    ...
    if (Tid.getText().trim().isEmpty()) {
        System.err.println("Error: Teacher ID is empty");
        return;
    }
    ...
    try {
        ...
        String getUsernameSQL = String.format(
            "SELECT CONCAT(ID, Name) AS username FROM teachers WHERE ID = %d", 
            teacherId);
        ...
        Teacher teacher = new Teacher();
        teacher.DeleteRecord(teacherId);
        
        if (username != null && !"root".equalsIgnoreCase(username)) {
            try {
                String escapedUser = username.replace("'", "''");
                String escapedDB = ESystem.currentDB.replace("'", "''");
                
                // First check if the user exists
                ResultSet userCheck = ESystem.st.executeQuery(
                    "SELECT 1 FROM mysql.user WHERE User = '" + escapedUser + "'");
                    
                if (userCheck.next()) {
                    // User exists, revoke privileges and drop
                    String revokeSQL = String.format(
                        "REVOKE ALL PRIVILEGES ON `%s`.* FROM '%s'@'%%';",
                        escapedDB, escapedUser);
                    ESystem.st.executeUpdate(revokeSQL);
                    
                    // Drop the user
                    String dropUserSQL = String.format("DROP USER IF EXISTS '%s'@'%%';", escapedUser);
        ...
        System.err.println("Error: Invalid teacher ID format");
    } catch (Exception ex) { 
        System.err.println("Error: " + ex.getMessage()); 
    }
}                                      
